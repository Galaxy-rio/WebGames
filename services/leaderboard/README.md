...
- **公开前 50 条**：每个榜单的公开查询只展示排序后的前 50 条成绩，默认分页为 20 / 20 / 10 条。边界同分也不会扩展到第 51 条；按达到成绩的时间与记录 ID 决定同分记录的展示顺序。第 50 条以后的成绩仍正常保存，管理员可以查询和管理全部记录。
...
`limit` 为 `1`–`100`，`offset` 为 `0`–`1000000` 的整数。公开读取始终限于每个榜单排序后的前 50 条记录，`total` 返回可见记录数（最多 `50`）。服务端会按剩余可见条数缩小实际查询范围；增大 `limit` 或直接修改 URL 不能读取第 51 条及之后的记录，`offset >= 50` 返回空列表。

响应中的 `limit` 保留请求的每页数量，所以 `limit=20`、`offset=0/20/40` 对应最多 `20/20/10` 条。边界同分仍按既有时间与记录 ID 稳定排序，只取前 50 条，并保留 `1、1、3` 的并列名次规则。未进入前 50 条的成绩可以正常提交、更新和保留；管理员查询返回全部匹配记录与真实 `total`，不受公开展示上限影响。关闭的游戏或榜单不再提供公开读取和提交。
