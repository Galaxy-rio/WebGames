...
| 空中飞行时间 | 每秒 −10，从首次控制飞船开始；停在星球上不扣 |
...
新对局在首次用鼠标或手指按下飞行画面前不计时；查看菜单和排行榜不会启动计时。首次控制后，松手滑行仍然计时。这个状态随存档保存；升级前已开始计时的旧存档继续计时，已扣分数保留。
...
复用 `services/leaderboard/` 的 Worker + D1 服务，游戏 ID 为 `landroid-extended`、榜单 ID 为 `exploration`。排行榜右下角的小齿轮打开当前接口对应的 `/admin/` 管理页面。迁移 `0002_landroid_extended.sql` 注册新榜单；服务端校验完成条件、AUTO 标记、奖励范围与计分算式，不包含服务器飞行回放验真。本地服务运行方式见根目录说明；发布时需执行新增 D1 迁移、更新 Worker，并在前端配置 `PUBLIC_LEADERBOARD_API`。
